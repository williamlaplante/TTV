...
def MH_gibbs(g : graph, num_samples=int(1e3), verbose=False, wandb_name = None):
    ...
    def accept(x : str, X : Env, X_p :Env) -> tc.tensor:
        d, _ = Eval(Q[x], {"logW":tc.tensor(0.0), "logP" : tc.tensor(0.0), "logJoint" : tc.tensor(0.0)}, X) #Evaluate expression of x (sample ...) given the values in the Env X
        d_p, _ = Eval(Q[x], {"logW":tc.tensor(0.0), "logP" : tc.tensor(0.0), "logJoint" : tc.tensor(0.0)}, X_p) #Evaluate expression of x given Env X_p
        ...
        for v in Vx:
            d1, _ = Eval(g.Graph["P"][v][1], {"logW":tc.tensor(0.0), "logP" : tc.tensor(0.0), "logJoint" : tc.tensor(0.0)}, X_p)
            d2, _ = Eval(g.Graph["P"][v][1], {"logW":tc.tensor(0.0), "logP" : tc.tensor(0.0), "logJoint" : tc.tensor(0.0)}, X)
    ...
    def Gibbs_step(X : Env) -> Env:
        ...
        for x in ordered_latent_vars:
            d, _ = Eval(Q[x], {"logW":tc.tensor(0.0), "logP" : tc.tensor(0.0), "logJoint" : tc.tensor(0.0)}, X) #get the distribution for the latent variable x
    ...
    for i, env in enumerate(X.values()):
        sample = Eval(g.Program, {"logW":tc.tensor(0.0), "logP" : tc.tensor(0.0), "logJoint" : tc.tensor(0.0)}, env)[0]
