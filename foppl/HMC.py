...
def sample_from_joint(g : graph) -> dict:
    ...
    for v in ordered_vars:
        if v in X: # if the vertex is latent, sample from the prior
            expr = g.Graph["P"][v]
            env[v], _ = Eval(expr, {"logW":tc.tensor(0.0), "logP" : tc.tensor(0.0), "logJoint" : tc.tensor(0.0)}, env)
...
def U(X : dict, Y : dict, g : graph)-> tc.tensor:
    ...
    for y in Y.keys():
        d, _ = Eval(g.Graph["P"][y][1], {"logW":tc.tensor(0.0), "logP" : tc.tensor(0.0), "logJoint" : tc.tensor(0.0)}, env)
        sigma["logW"] = sigma["logW"] + d.log_prob(tc.tensor(Y[y]).float())
    
    for x in X.keys():
        d, _ = Eval(g.Graph["P"][x][1], {"logW":tc.tensor(0.0), "logP" : tc.tensor(0.0), "logJoint" : tc.tensor(0.0)}, env)
...
def HMC_sampling(g, num_samples=int(1e3), T=10, eps=0.1, M_scale=1, verbose=False, wandb_name=None):
    ...
    for i, sample in enumerate(chain):
        ...
        ret, _ = Eval(g.Program, {"logW":tc.tensor(0.0), "logP" : tc.tensor(0.0), "logJoint" : tc.tensor(0.0)}, env)
